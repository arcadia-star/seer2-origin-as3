...
public class WalkRedCarpet {
    ...
    public function WalkRedCarpet() {
        super();
        DialogPanel.addEventListener(DialogPanelEvent.CUSTOM_REPLY_CLICK, this.onActivityUnitClick);
    ...
    private function onActivityUnitClick(param1:DialogPanelEvent):void {
        ...
        ItemManager.requestEquipList(this.hasEquipment);
        if ((evt.content as DialogPanelEventData).params == "Walk_Red") {
            if (DateUtil.inInDateScope(5, 13, 1, 14, 1)) {
                if (this._result) {
                    ...
                    NpcDialog.show(503, "神秘的某某", [[0, "额？你就这样就出门了？快去商城采购装备打扮一下再来走红毯吧！ "]], ["马上就去。"], [function ():void {
    ...
    public function dispose():void {
        DialogPanel.removeEventListener(DialogPanelEvent.CUSTOM_REPLY_CLICK, this.onActivityUnitClick);
        Connection.removeCommandListener(CommandSet.ITEM_EXCHANGE_1055, this.onRequestSwap);
    }
}
}
