...
public class MapProcessor_80416 extends MapProcessor {

    private static const FOR_LIST:Array = [205727];

    private static const DAY_LIST:Array = [1664];

    private static const FIGHT_INDEX_LIST:Vector.<int> = Vector.<int>([1534, 1535, 1536]);

    private static const RES_LIST:Vector.<int> = Vector.<int>([27, 141, 209]);
    ...
    public function MapProcessor_80416(param1:MapModel) {
        super(param1);
    }
        ...
        if (SceneManager.prevSceneType == SceneType.ARENA && FIGHT_INDEX_LIST.indexOf(FightManager.currentFightRecord.initData.positionIndex) != -1) {
            ActiveCountManager.requestActiveCountList(FOR_LIST, function (param1:Parser_1142):void {
                ...
                DayLimitListManager.getDaylimitList(DAY_LIST, function (param1:DayLimitListInfo):void {
                    ...
                    if (par.infoVec[0] >= 20) {
                        ServerMessager.addMessage("已获得足够的洛之力了哦!");
                        ...
                        SceneManager.changeScene(SceneType.LOBBY, 70);
                        TweenNano.delayedCall(2, function ():void {
                            ModuleManager.showAppModule("WorldKingFourGetPowerPanel");
                        });
                    }
                });
            });
    ...
    private function createNpc():void {
        ...
        if (!this._npc) {
            ...
            if (_loc1_ >= 0 && _loc1_ <= 50) {
                this._curIndex = 0;
            } else if (_loc1_ > 50 && _loc1_ <= 80) {
                this._curIndex = 1;
            } else {
                this._curIndex = 2;
            }
