package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_3852 extends MapProcessor
   {
      ...
      private static function onBroadCast(param1:MessageEvent) : void
      {
         ...
         ModelLocator.getInstance().dispatchEvent(new LogicEvent(LogicEvent.BEST_CATCH_PK_BROAD_CAST));
      }
      ...
      private function pkValHandle() : void
      {
         ...
         Connection.send(CommandSet.GET_TOTAL_VOTE_INFO_1219,PUBLIC_KEY,int(dateStr) * 10 + 1,int(dateStr) * 10 + 6);
         Connection.addCommandListener(CommandSet.GET_TOTAL_VOTE_INFO_1219,function getCallState(param1:MessageEvent):void
         {
            ...
            var _loc3_:uint = uint(_loc2_.readUnsignedInt());
            var _loc4_:uint = uint(_loc2_.readUnsignedInt());
            var _loc5_:Vector.<uint> = Vector.<uint>([int(dateStr) * 10 + 1,int(dateStr) * 10 + 2,int(dateStr) * 10 + 3,int(dateStr) * 10 + 4,int(dateStr) * 10 + 5,int(dateStr) * 10 + 6]);
            ...
            while(_loc7_ < _loc4_)
            {
               ...
               if(_loc5_.indexOf(_loc10_) != -1)
               {
                  _loc6_[_loc5_.indexOf(_loc10_)] = _loc11_;
               }
               _loc7_++;
            }
            var _loc8_:Array = [];
            ...
            else
            {
               ...
               _loc8_.push(_loc6_[4]);
               _loc8_.push(_loc6_[5]);
            }
            _scroeNumList[0].text = _loc8_[0].toString();
            _scroeNumList[1].text = _loc8_[1].toString();
            _scroeNumList[2].text = _loc8_[2].toString();
            var _loc9_:uint;
            if((_loc9_ = uint(int(_loc8_[0]) + int(_loc8_[1]) + int(_loc8_[2]))) == 0)
            {
               _loc9_ = 1;
            }
            _barList[0].scaleX = int(_scroeNumList[0].text) / _loc9_;
            _barList[1].scaleX = int(_scroeNumList[1].text) / _loc9_;
            _barList[2].scaleX = int(_scroeNumList[2].text) / _loc9_;
         });
      }
      ...
      private function addListener() : void
      {
         ...
         ModelLocator.getInstance().addEventListener(LogicEvent.BEST_CATCH_PK_BROAD_CAST,this.onPkModify);
      }
      ...
      private function addUser(param1:MessageEvent) : void
      {
         ...
         if(_loc3_ != ActorManager.actorInfo.id)
         {
            if(_loc4_ = ActorManager.getRemoteActor(_loc3_))
            {
               ...
               this.setInitPosByTeam(_loc4_,_loc4_.getInfo().activityData[20]);
            }
         }
      }
