...
public class MapProcessor_80160 extends MapProcessor {


    private const NPC_ID:uint = 729;

    private const FIGHT_COUNT:uint = 958;

    private const BUY_COUNT:uint = 203898;

    private const BUY_COUNT_ID:uint = 603594;

    private var leftCount:int;

    private var npc:Mobile;

    public function MapProcessor_80160(param1:MapModel) {
        super(param1);
    }
    ...
    protected function toFight(param1:DialogPanelEvent):void {
        if (this.leftCount == 0) {
            AlertManager.showConfirm("剩余挑战次数不足!", this.toConfirm);
            return;
        }
        if ((param1.content as DialogPanelEventData).params == "toFight") {
            ...
            ModuleManager.showAppModule("FightBossWithNaJiaPanel");
        }
    }
    ...
    private function toConfirm():void {
        ...
        _loc1_.itemId = this.BUY_COUNT_ID;
