...
public class MapProcessor_80412 extends MapProcessor {

    private static const FOR_LIST:Array = [205698];

    private static const DAY_LIST:Array = [1654];

    private static const FIGHT_INDEX_LIST:Vector.<int> = Vector.<int>([1526, 1527, 1528]);

    private static const RES_LIST:Vector.<int> = Vector.<int>([351, 428, 587]);

    private static const LEVEL_LIST:Vector.<int> = Vector.<int>([60, 20, 100]);
    ...
    public function MapProcessor_80412(param1:MapModel) {
        super(param1);
    }
        ...
        if (SceneManager.prevSceneType == SceneType.ARENA && FIGHT_INDEX_LIST.indexOf(FightManager.currentFightRecord.initData.positionIndex) != -1) {
            ActiveCountManager.requestActiveCountList(FOR_LIST, function (param1:Parser_1142):void {
                ...
                DayLimitListManager.getDaylimitList(DAY_LIST, function (param1:DayLimitListInfo):void {
                    ...
                    if (par.infoVec[0] >= 20) {
                        ServerMessager.addMessage("已获得足够的焰之力了哦!");
                        ...
                        SceneManager.changeScene(SceneType.LOBBY, 70);
                        TweenNano.delayedCall(2, function ():void {
                            ModuleManager.showAppModule("WorldKingThreeGetPowerPanel");
                        });
                    }
                });
            });
