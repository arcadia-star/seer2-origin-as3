package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80341 extends MapProcessor
   {
      
      private static const FOR_LIST:Array = [205089,205064,205065];
      
      private static const DAY_LIST:Array = [1448];
      
      private static const FIGHT_INDEX:int = 1331;
      
      private static const MI_ID_LIST:Vector.<uint> = Vector.<uint>([605130,500619,605134]);
      ...
      public function MapProcessor_80341(param1:MapModel)
      {
         super(param1);
      }
      ...
      private function initSet() : void
      {
         ...
         if(this._par)
         {
            if(BitUtil.getBit(this._par.infoVec[2],3))
            {
      ...
      private function requestNum() : void
      {
         ItemManager.addEventListener1(ItemEvent.REQUEST_SPECIAL_ITEM_SUCCESS,this.getItemNum);
         ItemManager.requestSpecialItemList();
      }
      
      private function getItemNum(param1:ItemEvent) : void
      {
         var num:int;
         var evt:ItemEvent = param1;
         ItemManager.removeEventListener1(ItemEvent.REQUEST_SPECIAL_ITEM_SUCCESS,this.getItemNum);
         num = ItemManager.getSpecialItem(MI_ID_LIST[1]) != null ? int(ItemManager.getSpecialItem(MI_ID_LIST[1]).quantity) : 0;
         this._hurtBuy.mouseEnabled = true;
         if(num > 0)
         {
            AlertManager.showAlert("圣冰荣耀使用完再来购买吧!");
         }
         else
         {
            AlertManager.showConfirm("你确定花费" + ShopPanelConfig.getItemPrice(MI_ID_LIST[1]) + "星钻购买圣冰荣耀吗?",function():void
            {
      ...
      private function createNpc() : void
      {
         if(!this._npc)
         {
            ...
            this._npc.resourceUrl = URLUtil.getNpcSwf(767);
            this._npc.labelPosition = MobileLabelPosition.OVER_HEAD;
            this._npc.label = "圣灵天尊";
