package com.taomee.seer2.app.processor.copy.handler
{
   ...
   public class CopyProcessor_80015 extends CopyProcessor
   {
      ...
      {
         StatisticsManager.sendNovice(StatisticsManager.ui_interact_426);
      }
         ...
         if(SceneManager.prevSceneType == SceneType.ARENA)
         {
            if(FightManager.fightWinnerSide == FightSide.LEFT)
            {
               ...
               if(index == this.MONSTER_INDEX)
               {
                  ...
                  {
                     ServerMessager.addMessage("你还需要击败" + (this.GOLE_KILL_NUM - this._killNum) + "只精灵就可以和泰瑞纳斯对战！");
                  }
                  else
                  {
      ...
      private function onBossClick(param1:MouseEvent) : void
      {
         ...
         switch(param1.currentTarget)
         {
            case this._stateOneBoss:
               if(this._killNum < this.GOLE_KILL_NUM)
               {
                  AlertManager.showAlert("你需要击败" + (this.GOLE_KILL_NUM - this._killNum) + "只精灵就可以和泰瑞纳斯对战！");
               }
               break;
