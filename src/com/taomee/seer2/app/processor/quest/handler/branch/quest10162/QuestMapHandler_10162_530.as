...
public class QuestMapHandler_10162_530 extends QuestMapHandler {


    public function QuestMapHandler_10162_530(param1:QuestProcessor) {
        super(param1);
    }
    ...
    private function doWin():void {
        OnlyFlagManager.RequestFlag(function ():void {
            if (OnlyFlagManager.getFlag(630) != 1) {
                SwapManager.swapItem(917, 1, onSwap);
            }
        });
    }
    ...
    private function getItemHandle(param1:ItemEvent):void {
        ...
        if (ItemManager.getSpecialItem(500544)) {
            num = int(ItemManager.getSpecialItem(500544).quantity);
            if (num >= 10) {
                OnlyFlagManager.RequestFlag(function ():void {
                    if (OnlyFlagManager.getFlag(630) == 0) {
                        SwapManager.swapItem(922, 1, onSwapHandler);
                    }
                });
            } else {
                ServerMessager.addMessage("恭喜你获得1个巨锤莫古碳甲！还需要" + (10 - num) + "个");
            }
        }
    }
    ...
    private function onSwapHandler(param1:IDataInput):void {
        ...
        OnlyFlagManager.updataFlag(630, 1);
