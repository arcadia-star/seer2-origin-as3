...
public class QuestProcessor_10057 extends QuestProcessor {


    public var count:int = 0;

    public function QuestProcessor_10057(param1:Quest) {
        super(param1);
    ...
    private function onSwictchComplete(param1:SceneEvent):void {
        ...
        if (_loc2_ && QuestManager.isAccepted(_quest.id) && SceneManager.prevSceneType == SceneType.ARENA) {
            if (FightManager.currentFightRecord && FightManager.currentFightRecord.initData && FightManager.currentFightRecord.initData.bossId == 6) {
                ++this.count;
            }
        }
    }
