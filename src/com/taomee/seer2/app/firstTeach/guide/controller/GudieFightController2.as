...
public class GudieFightController2 implements IFightController {
    ...
    public function GudieFightController2(param1:ArenaScene) {
    ...
    private function parseTurn():void {
        ...
        if (_loc1_) {
            if (SceneManager.currentSceneType == SceneType.GUDIE_ARENA3 && _loc1_.notifyIndex == 1) {
                this._scene.fightController.exitFight();
            } else if (SceneManager.currentSceneType == SceneType.GUIDE_NEW_ARENA1 && _loc1_.notifyIndex == 1) {
                this._scene.fightController.exitFight();
            } else if (SceneManager.currentSceneType == SceneType.GUDIE_ARENA4 && _loc1_.notifyIndex == 1) {
                this._scene.fightController.exitFight();
            } else if (SceneManager.currentSceneType == SceneType.GUIDE_NEW_ARENA2 && _loc1_.notifyIndex == 1) {
                this._scene.fightController.exitFight();
            } else if (SceneManager.currentSceneType == SceneType.GUDIE_ARENA2) {
                if (_loc1_.notifyIndex == 1) {
                    GudieFightTipContent.startTip("UI_TIP1", 1, this._par, true);
        ...
        onAnimationEnd = function ():void {
            ...
            if (count <= 0) {
                if (SceneManager.currentSceneType == SceneType.GUDIE_ARENA2) {
                    ...
                    mc = FightUIManager.getMovieClip("UI_QUEST_NEW");
                    if (_par) {
                        _par.addChild(mc);
                    }
                    currIndex = 0;
                    mc.addEventListener(MouseEvent.CLICK, function (param1:MouseEvent):void {
                        if (currIndex == 0) {
                            ...
                            mc.gotoAndPlay(mc.totalFrames - 1);
                        }
                        ++currIndex;
                    });
                    ...
                    _scene.arenaUIController.startSelectOperate();
                    mc = FightUIManager.getMovieClip("UI_QUEST_NEW3");
                    if (_par) {
                        _par.addChild(mc);
                    }
                    currIndex = 0;
                    ...
                    _scene.arenaUIController.startSelectOperate();
                    GudieFightTipContent.startTip("UI_TIP5", 5, _par);
    ...
    private function gudie5():void {
        GudieFightTipContent.startTip("UI_TIP6", 6, this._par);
    }

    private function gudie4():void {
        var mc:MovieClip = null;
        var currIndex:int = 0;
        mc = FightUIManager.getMovieClip("UI_QUEST_NEW2");
        if (this._par) {
            this._par.addChild(mc);
        }
        currIndex = 0;
        mc.addEventListener(MouseEvent.CLICK, function (param1:MouseEvent):void {
            if (currIndex == 0) {
