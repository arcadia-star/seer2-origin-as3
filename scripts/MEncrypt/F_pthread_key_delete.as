package MEncrypt
{
   import MEncrypt__3A__5C_alchemy_5C_cygwin_5C_tmp_5C_cc80XT98_2E_lto_2E_bc_3A_1b3fc948_2D_82a0_2D_4fee_2D_a9ab_2D_31c892a97f18.*;
   import avm2.intrinsics.memory.*;
   
   public function F_pthread_key_delete() : void
   {
      var _loc3_:* = 0;
      var _loc9_:* = 0;
      var _loc1_:* = 0;
      var _loc5_:* = 0;
      var _loc7_:* = 0;
      var _loc6_:int = 0;
      var _loc2_:* = ESP;
      _loc3_ = _loc2_;
      _loc1_ = -1;
      if((_loc9_ = li32(_loc3_)) >= 0)
      {
         var _loc8_:*;
         if((_loc8_ = li32(_sEntryCount)) > _loc9_)
         {
            _loc1_ = li32((_loc6_ = li32(_sEntries) + (_loc7_ = _loc9_ << 3)) + 4);
            if(_loc1_ != 0)
            {
               if((_loc5_ = li32(_loc6_)) != 0)
               {
                  _loc2_ -= 16;
                  si32(_loc5_,_loc2_);
                  ESP = _loc2_;
                  ptr2fun[_loc1_]();
                  _loc2_ += 16;
               }
            }
            _loc1_ = 0;
            si32(_loc1_,_loc6_);
            _loc8_ = (_loc8_ = li32(_sEntries)) + _loc7_;
            var _loc4_:*;
            si32(_loc4_ = li32(_sFirstFree),_loc8_ + 4);
            si32(_loc9_,_sFirstFree);
         }
      }
      eax = _loc1_;
      _loc2_ = _loc3_;
      ESP = _loc2_;
   }
}
