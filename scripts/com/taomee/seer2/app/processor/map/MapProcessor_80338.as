package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80338 extends MapProcessor
   {
      
      private static const FOR_LIST:Array = [205065];
      
      private static const DAY_LIST:Array = [1444,1447];
      
      private static const FIGHT_INDEX:int = 1329;
      
      private static const LAST_TIME:int = 300;
      
      private static const RES_ID:int = 271;
      
      private static const MI_ID_LIST:Vector.<uint> = Vector.<uint>([605127]);
      ...
      public function MapProcessor_80338(param1:MapModel)
      {
         super(param1);
      }
      ...
      private function update() : void
      {
         ActiveCountManager.requestActiveCountList(FOR_LIST,function(param1:Parser_1142):void
         {
            ...
            DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
            {
               ...
               {
                  if(_info.getCount(DAY_LIST[1]) < param1.getCount(DAY_LIST[1]))
                  {
                     ServerMessager.addMessage("恭喜本次挑战获得" + (param1.getCount(DAY_LIST[1]) - _info.getCount(DAY_LIST[1])) + "分哦!");
                  }
                  _info = param1;
               }
               ...
               _scroe.text = _info.getCount(DAY_LIST[1]).toString();
               _leaveTime = _info.getCount(DAY_LIST[0]) - TimeManager.getPrecisionServerTime();
               if(_leaveTime <= 0)
               {
                  _leaveTime = 0;
               }
               ...
               if(_leaveTime > 0)
               {
                  ...
                  else if(_info.getCount(DAY_LIST[1]) < 50)
                  {
                     createMonster();
                  }
               }
      ...
      private function createMonster() : void
      {
         ...
         while(_loc1_ < 3)
         {
            this._monsterList.push(new SpawnedPet(RES_ID,FIGHT_INDEX,0));
