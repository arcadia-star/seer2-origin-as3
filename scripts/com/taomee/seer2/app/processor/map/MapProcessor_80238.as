package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80238 extends MapProcessor
   {
      
      private static const FOR_LIST:Array = [204515,204511,204514,204513];
      
      private static const DAY_LIST:Array = [1264];
      
      private static const FIGHT_INDEX:int = 1135;
      
      private static const MI_ID_LIST:Vector.<uint> = Vector.<uint>([604033,500611,604036]);
      ...
      public function MapProcessor_80238(param1:MapModel)
      {
         super(param1);
      }
      ...
      private function onHurtBuy(param1:MouseEvent) : void
      {
         ...
         ActiveCountManager.requestActiveCountList(FOR_LIST,function(param1:Parser_1142):void
         {
            ...
            {
               AlertManager.showConfirm("你确定花费" + ShopPanelConfig.getItemPrice(MI_ID_LIST[1]) + "星钻购买影皇荣耀吗?",function():void
               {
      ...
      private function createNpc() : void
      {
         if(!this._npc)
         {
            ...
            this._npc.resourceUrl = URLUtil.getNpcSwf(726);
      ...
      private function isCanFight() : Boolean
      {
         ...
         {
            if([18,12,8].indexOf(_loc2_.type) != -1)
            {
               _loc1_ = false;
               AlertManager.showAlert("不能携带特质、光、电系精灵上场，重新调整后再来挑战哦！");
               break;
            }
         }
         return _loc1_;
      ...
      private function buyComplete(param1:Parser_1224) : void
      {
         ...
         buyInfo.itemId = MI_ID_LIST[0];
         buyInfo.buyComplete = this.buyComplete;
         ShopManager.buyVirtualItem(buyInfo);
         ServerMessager.addMessage("恭喜通关喽!");
