package com.taomee.seer2.app.processor.quest.handler.branch.quest10090
{
   ...
   public class QuestMapHandler_10090_191 extends QuestMapHandler
   {
      ...
      public function QuestMapHandler_10090_191(param1:QuestProcessor)
      {
         super(param1);
      }
         ...
         if(QuestManager.isAccepted(_quest.id) && QuestManager.isStepComplete(_quest.id,1) == false)
         {
            if(SceneManager.prevSceneType == SceneType.ARENA && FightManager.fightWinnerSide == FightSide.LEFT)
            {
               if(FightManager.currentFightRecord && FightManager.currentFightRecord.initData && Boolean(FightManager.currentFightRecord.initData.positionIndex) && Boolean(FightManager.currentFightRecord.initData.type) && FightManager.currentFightRecord.initData.type == FightRecordData.FIGHT_WILD)
               {
                  ...
                  if(SceneManager.active.mapID == 191 && _loc1_ >= 0 && _loc1_ <= 9)
                  {
                     this.executeKe();
                  }
               }
      ...
      private function executeKe() : void
      {
         if(DateUtil.inInDateScope(6,14,30,15,1))
         {
            ...
            {
               ++this._fightNum2;
               _quest.setStepData(_quest.getCurrentOrNextStep().id,1,this._fightNum2);
               QuestManager.addEventListener(QuestEvent.STEP_UPDATE_BUFFER,this.onStepUpdateBuffer2);
               QuestManager.setStepBufferServer(_quest.id,_quest.getCurrentOrNextStep().id);
            }
         }
      }
      
      private function onStepUpdateBuffer2(param1:QuestEvent) : void
      {
         if(param1.questId == _quest.id)
         {
            this._fightNum1 = _quest.getStepData(_quest.getCurrentOrNextStep().id,0);
            this._fightNum2 = _quest.getStepData(_quest.getCurrentOrNextStep().id,1);
            QuestManager.removeEventListener(QuestEvent.STEP_UPDATE_BUFFER,this.onStepUpdateBuffer2);
            if(this._fightNum2 < 5)
            {
               ServerMessager.addMessage("已经得到了" + this._fightNum2 + "个保护罩,还需要打" + (5 - this._fightNum2).toString() + "只安可.");
            }
            else if(this._fightNum2 == 5)
            {
               ServerMessager.addMessage("已经得到了5个保护罩.");
